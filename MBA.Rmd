...
title: "MBA"
author: "Lee Marshall"
date: "2021_10_28 - `r format(Sys.time(), '%Y_%m_%d')`"
output:
...
# MBA {.tabset .tabset-fade .tabset-pills}
...
```{r Nutil_Whole_Brain_Average}
#---------- Nutil_Whole_Brain_Average
sample_path <- "Nutil_Outputs"
sample_ids <- list.files(path = sample_path)

#----- Nutil_Whole_Brain_Average 
sample_id <- sample_ids[1]
sample_id_slices <- list.files(path = paste(sample_path,sample_id,
                                           sep = "/")) %>% 
  stringr::str_extract("[0-9]{2}") %>% unique()

sample_id_slice1 <- read_delim(file = paste(sample_path, 
                                            sample_id, 
                                            paste0("Figure ", 
                                                   sample_id_slices[1], 
                                                   ".txt"),
...
sample_id_slice2 <- read_delim(file = paste(sample_path, 
                                            sample_id, 
                                            paste0("Figure ", 
                                                   sample_id_slices[1], 
                                                   " REPEAT.txt"), 
...
sample_id_slice <- merge(x = sample_id_slice1, 
                         y = sample_id_slice2, 
                         by = c("Region_ID", "Region_Name"))
sample_id_slice$Object_Percentage_Mean <- rowMeans(sample_id_slice[,c(-1:-2)])


library(xml2)
MBA_1 <- read_xml("MBA_1.svg")
MBA_1list <- as_list(MBA_1)
...
for(i in seq(length(sample_ids))){
  ...
  id_files <- list.files(path = paste(sample_path,sample_id,sep = "/"))
  for(j in seq(length(id_files))){
    if(j == 1){
      id_sections <- read_delim(paste(sample_path,sample_id,id_files[j],sep="/"), 
                                delim = "\t", 
                                escape_double = FALSE, 
                                trim_ws = TRUE) %>% 
      ...
      id_sections <- rbind(id_sections,
                           read_delim(paste(sample_path,sample_id,id_files[j],sep="/"), 
                                      delim = "\t", 
                                      escape_double = FALSE, 
                                      trim_ws = TRUE) %>% 
                             rename(Region_Name = `Region Name`) %>% 
